...
qtcr_path = '/home/marisolbarrientosmoreno/Desktop/Summer_Semester/i17/deadline/regulatory-documents-tie/temporal_compliance_verificator/data/input/to_evaluate/step_8/'
...
def date_in_between(start, end, date):
    if start.month <= date.month <= end.month:
        return True
    else:
        return False
...
def read_qrtc(desired):
    all.remove(desired)
    df = pd.read_csv(qtcr_path + 'auto_step_8.csv')
...
def before_restriction(timestamps, time_restriction):
    for timestamp in timestamps:
        timestamp = datetime.strptime(timestamp[:19], '%Y-%m-%d %H:%M:%S')
        if timestamp.hour < time_restriction.hour or timestamp.hour == time_restriction.hour:
            return True
    return False
...
def in_restriction(timestamps, time_restriction):
    for timestamp in timestamps:
        timestamp = datetime.strptime(timestamp[:19], '%Y-%m-%d %H:%M:%S')
        if timestamp.hour == time_restriction.hour:
            return True
    return False
...
def in_restriction_flex(timestamps, time_restriction, flexibility):
    for timestamp in timestamps:
        timestamp = datetime.strptime(timestamp[:19], '%Y-%m-%d %H:%M:%S')
        if timestamp.hour == time_restriction.hour or timestamp.hour == time_restriction.hour  + flexibility or timestamp.hour + flexibility == time_restriction.hour:
            return True
    return False
...
def loop_logs(process_description):
    ...
    #verify_timestamps(process_description, log_name)
...
def verify_timestamps(qrtc_file, log, log_name, process_description):
    ...
    for index, row in qrtc_file.iterrows():
        ...
        if reference != 'NO':
            ...
            if reference == 'START':
                reference_time = timestamp_first_activity + timedelta(
                    get_time(reference_shift, reference_granularity).hour)
            else:
        ...
        if activity_to == 'END':
            ...
            activity_to = reference_time - timedelta(get_time(reference_shift, reference_granularity).hour)
        else:
            ...
            if not condition_meet:
                ...
                if status == '*':
                    timestamps = found_activities.loc[found_activities['lifecycle:transition'] == 'complete'][
                        'shift:timestamp']
                    if not before_restriction(timestamps, activity_to):
...
def print_csv(violations):
    ...
    cvs_file_name = '/home/marisolbarrientosmoreno/Desktop/Summer_Semester/i17/deadline/regulatory-documents-tie/temporal_compliance_verificator/data/output/to_evaluate/step_8/qtcr_auto_step_8.csv'
    newfile = False
